...
typedef struct {
  ...
  int           size_top;	/* total size of top shmseg             */
  int           size_seg;	/* total size of linked shmseg          */
  int           size_data;	/* available data size in top shmseg    */
  int           size_chunk;	/* available data size in linked shmseg */
...
Share	*sharelite_attach(key_t key, int segsize, int flags);
...
/* set the size of any subsequent linked segments */
int	 sharelite_segsize(Share *share, size_t size);
